...
export async function eliminaPrenotazione(req: Request, res: Response) {
  ...
  const prenotazioneId: number = parseInt(req.params.id, 10);

  try {
    // Verifica se l'utente è autorizzato
    const userToken: string = req.headers.authorization || '';
    const adminToken: string = req.headers.authorization || '';
    ...
    const [results] = await connection.promise().query(query, [prenotazioneId]);
    ...
    // Gestisci gli errori
    return res.status(500).json({ success: false, message: 'Errore durante l\'eliminazione della prenotazione', error: (err as any).message });
  }
}
